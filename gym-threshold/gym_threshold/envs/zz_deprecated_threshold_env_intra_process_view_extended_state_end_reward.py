...
class ThresholdEnvIntraProcessExtendedEndReward(BaseEnv):
    ...
    def __init__(self):
        ...
        low_array = np.array([0., 0., 0., 0.])
        high_array = np.array([23., 23., 1.1, 2.])
    ...
    def step(self, action=None):
        ...
        info = {}
        self.state = np.array(
            [self.position, self.process_length, self.reliability, self.predicted_duration])
    ...
    def compute_reward(self, adapted, cost, done, predicted_duration, planned_duration, reliability, position,
        ...
        if done:
            ...
            self.rewards_per_episode.append(-(50. / process_length))
            reward = 0
